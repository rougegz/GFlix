...
import com.gflix.app.activities.main.MainTvActivity
...
import com.gflix.app.utils.WebSocketBypassTestHelper
...
class SettingsTvFragment : LeanbackPreferenceFragmentCompat() {
    ...
    private fun displaySettings() {
        ...
        findPreference<Preference>("key_backup_export_tv")?.setOnPreferenceClickListener {
            ...
            val fileName = "gflix_tv_backup_$timestamp.json"
        ...
        findPreference<Preference>("key_backup_export_db_tv")?.setOnPreferenceClickListener {
            val timestamp = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(Date())
            val fileName = "gflix_tv_db_backup_$timestamp.zip"
            ...
            withBackupLoading(R.string.backup_export_title) {
                ...
                runCatching {
                    val values = ContentValues().apply {
                        ...
                        put(MediaStore.Downloads.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS + "/GFlix")
                    }
                    ...
                    Toast.makeText(
                        requireContext(),
                        getString(R.string.backup_export_saved_to, "Downloads/GFlix/$fileName"),
                        Toast.LENGTH_LONG
                    ).show()
                }.onFailure { error ->
                    Toast.makeText(requireContext(), getString(R.string.backup_export_error_write), Toast.LENGTH_LONG).show()
