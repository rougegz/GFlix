...
class PlayerViewModelV2(
    ...
    fun resolve() = viewModelScope.launch(Dispatchers.IO) {
        ...
        try {
            val all = mutableListOf<ExtLink>()
            for (api in apis()) {
                runCatching { api.loadLinks(dataUrl) }
                    .onSuccess { all += it }
            }
            val sorted = CloudStreamAdapter.sortBestFirst(all)
            if (sorted.isEmpty()) throw IllegalStateException("No links from extensions")
            val videos = sorted.map { CloudStreamAdapter.toVideo(it) }
